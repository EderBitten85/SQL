const ModeloCliente = require('./../model_DAO/ClienteModel_DAO.js');
...
class ControladorCliente{
    ...
    async obterTodosClientes( req, res ){
        try{
            const cliente = await ModeloCliente.obterTodosClientes();
            res.status(200).json(cliente);
        } catch( erro ){
            res.status(500).json({ erro: 'Erro ao buscar todos os clientes teste'});
        }
    }
